/*************************************************************************
	> File Name: 3.haffma.cpp
	> Author: 
	> Mail: 
	> Created Time: Wed 25 Dec 2024 08:39:27 PM CST
 ************************************************************************/
...
#define MAX_CHAR 128
char *char_code[MAX_CHAR] = {0};

void extractHaffmanCode(Node *root, char buff[], int k) {
	buff[k] = 0;
	if(root->lchild == NULL && root->rchild == NULL) {
		char_code[root->ch] = strdup(buff);//把传入的字符串拷贝一份，赋值给前面的字符指针
		return ;
	}
	buff[k] = '0';
