/*************************************************************************
	> File Name: 2.list_file_with_info.c
	> Author: 
	> Mail: 
	> Created Time: Thu 05 Dec 2024 07:46:09 PM CST
 ************************************************************************/
...
int main(int argc, char **argv)
{
    ...
    while((entry = readdir(dir)) != NULL) {
        if(entry->d_name[0] != '.'){
            ...
            //1.函数内部malloc申请空间,在外面释放
            //2.静态字符数组
            printf("%s %ld %s %s %6ld %s ", mode_to_str(file_stat.st_mode), file_stat.st_nlink
                   , uid_to_name(file_stat.st_uid), gid_to_name(file_stat.st_gid), file_stat.st_size,       
                   time_to_str(file_stat.st_mtime));
            printf("%s\n",  entry->d_name);
        }
    }
    return 0;
